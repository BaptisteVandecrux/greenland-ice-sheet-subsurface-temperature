...
ds = xr.open_dataset(
    "C:/Users/bav/OneDrive - Geological survey of Denmark and Greenland/Data/AWS/GC-Net/20190501_jaws/08c.dat_Req1957.nc"
)
df = ds.to_dataframe()
df = df.reset_index(1)
...
ax[0].plot(
    np.arange(-18, 2, 0.5), p(np.arange(-18, 2, 0.5)), label="correction function"
)
...
ax[0].annotate(
    "ME = %0.2f\nRMSE = %0.2f" % (np.mean(X - Y), np.sqrt(np.mean((X - Y) ** 2))),
    (-16, -2.5),
)
...
ax[1].annotate(
    "ME = %0.2f\nRMSE = %0.2f"
    % (np.round(np.mean(p(X) - Y), 2), np.sqrt(np.mean((p(X) - Y) ** 2))),
    (-16, -2.5),
)
plt.tight_layout()
# %%
...
for site in sites:
    ...
    i = 0
    for i in range(23):
        if (df_firncover["depth_" + str(i)].iloc[0] < 0.1) | (
            np.isnan(df_firncover["depth_" + str(i)].iloc[0])
        ):
            continue
        df_firncover["depth_" + str(i)].plot(ax=ax[0])
        df_firncover["rtd" + str(i)].plot(ax=ax[1], label="FirnCover")
