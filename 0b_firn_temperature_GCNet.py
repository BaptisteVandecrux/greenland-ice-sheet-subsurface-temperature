...
for ID, site in zip(df_info.ID, df_info.Name):   
    df_aws = read(path_to_GCNet+'/hourly/'+site.replace(' ','')+'.csv').to_dataframe()
    df_aws['time'] = pd.to_datetime(df_aws.timestamp, utc=True)
    df_aws = df_aws.set_index('time')
    print(site)
...
# saving T10m
df_gcn['depthOfTemperatureObservation'] = 10
df_all = df_gcn.rename(columns={'TS_10m':'temperatureObserved',
