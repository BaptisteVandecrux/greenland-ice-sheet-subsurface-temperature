...
lat = [69.8798, 66.4800, 66.4797, 65.9995, 63.1489, 75.0000, 73.8419, 72.5797, 78.0168]
lon = [
    -46.9867,
    -46.2789,
    -42.5002,
...
for ii, site in progressbar.progressbar(enumerate(sites)):
    ...
    df_5 = ftl.interpolate_temperature(
        df_d["date"],
        df_d[["depth_" + str(i) for i in range(1, 11)]].values,
        df_d[["rtd" + str(i) for i in range(1, 11)]].values,
        5,
    ).set_index("date")
    df_15 = ftl.interpolate_temperature(
        df_d["date"],
        df_d[["depth_" + str(i) for i in range(1, 11)]].values,
        df_d[["rtd" + str(i) for i in range(1, 11)]].values,
        15,
    ).set_index("date")
    df_20 = ftl.interpolate_temperature(
        df_d["date"],
        df_d[["depth_" + str(i) for i in range(1, 11)]].values,
        df_d[["rtd" + str(i) for i in range(1, 11)]].values,
        20,
