...
ds = xr.open_dataset('C:/Data_save/RCM/HIRHAM/RetMIP_3Doutput_Annual_DMIHH_temp_1980-2016.nc')
...
for i, (lat,lon) in progressbar.progressbar(enumerate(zip(df.latitude,df.longitude))):
    if lat == lat_prev and lon == lon_prev:
        df.iloc[i,df.columns.get_loc('HIRHAM_lat')] = df.iloc[i-1, df.columns.get_loc('HIRHAM_lat')]
        df.iloc[i,df.columns.get_loc('HIRHAM_lon')] = df.iloc[i-1, df.columns.get_loc('HIRHAM_lon')]
        df.iloc[i,df.columns.get_loc('HIRHAM_distance')] = df.iloc[i-1, df.columns.get_loc('HIRHAM_distance')]
        df.iloc[i,df.columns.get_loc('HIRHAM_i')] = df.iloc[i-1, df.columns.get_loc('HIRHAM_i')]
        df.iloc[i,df.columns.get_loc('HIRHAM_j')] = df.iloc[i-1, df.columns.get_loc('HIRHAM_j')]
        df.iloc[i,df.columns.get_loc('HIRHAM_elevation')] =  df.iloc[i-1, df.columns.get_loc('HIRHAM_elevation')]
    else:
        ...
        df.iloc[i, df.columns.get_loc('HIRHAM_distance')] = cdist([(lon,lat)], [closest]) [0][0]
...
time_HIRHAM = pd.to_datetime([int(s) for s in np.floor(ds['time'].values)],format='%Y%M%d')
df = df.reset_index(drop=True)
for ind in progressbar.progressbar(df.index):   
    df.loc[ind,'HIRHAM_10m_temp'] = ds.temp[dict(
                x=int(df.loc[ind].HIRHAM_j),
                y=int(df.loc[ind].HIRHAM_i),
                level=10,
                time=np.argmin(np.abs(time_HIRHAM - np.datetime64(df.loc[ind,'date'])))
                )].values-273.15
# %% 
...
for i, ref in enumerate(ref_list):
    ax.plot(df_10m.loc[df_10m['reference_short']==ref,:].HIRHAM_10m_temp,
...
for data_name in dataset:
    ...
    for i, site in enumerate(site_list):
        ax.plot(df_10m.loc[df_10m['site']==site,:].HIRHAM_10m_temp,
...
for data_name in dataset:
    ...
    for i, site in enumerate(site_list):
        ...
        tmp.temperatureObserved.plot(ax =ax,
                       ...
                       color=cmap(i/len(site_list)), 
                       label = site + ' obs' )
        tmp.HIRHAM_10m_temp.plot(ax=ax,
...
#%%
# date	site	latitude	longitude	elevation	depthOfTemperatureObservation	temperatureObserved	reference	reference_short	note	year	geometry	HIRHAM_distance	HIRHAM_lat	HIRHAM_lon	HIRHAM_i	HIRHAM_j	HIRHAM_elevation	HIRHAM_10m_temp
...
for year in  range(2017,2020):
    ...
    # ds = xr.open_dataset('I:\Baptiste\Data\RCM\HIRHAM\HIRHAMv3.12_fixed\HIRHAMv3.12-10km-daily-ERA5-'+str(year)+'.nc')
    ds = xr.open_dataset('I:/Baptiste/Data/RCM/HIRHAM/b92/ICE.'+str(year)+'.01-12.b92.nc')
    points = [(x, y) for x,y in zip(ds['LAT'].values.flatten(),ds['LON'].values.flatten())]

    closest = points[cdist([(lat,lon)], points).argmin()]
    HIRHAM_lat = closest[0]
    HIRHAM_lon = closest[1]
    HIRHAM_distance = cdist([(lon,lat)], [closest]) [0][0]
    ...
    ds_ice = ds.TI1[dict(X10_85=HIRHAM_j, Y20_155=HIRHAM_i)].reset_coords() 
    ds_ice = ds_ice.load()
...
plt.xlabel('time')
plt.title('HIRHAM output at T-16: x=-200.0 (index 44), y=-2497.9277 (index 85)')
