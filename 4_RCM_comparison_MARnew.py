...
ds = xr.open_dataset(
    "I:/Baptiste/Data/RCM/MAR/" + mar_ver + "/ICE.2007.01-12." + mar_ver + ".nc",
    decode_times=False,
)
points = [
...
for year in years:
    ...
    for ind in progressbar.progressbar(df_year.index):
        df_year.loc[ind, "MAR_10m_temp"] = ds.TI1[
            dict(
                X10_85=int(df_year.loc[ind].MAR_j),
                Y20_155=int(df_year.loc[ind].MAR_i),
                OUTLAY=15,
                TIME=np.argmin(np.abs(time_mar - np.datetime64(df_year.loc[ind].date))),
            )
        ].values
    df.loc[df.year == year, :] = df_year.values
# %%
df.to_csv("subsurface_temperature_summary_w_MAR_" + mar_ver + ".csv")

# %% Comparison
df = pd.read_csv("subsurface_temperature_summary_w_MAR_" + mar_ver + ".csv")
...
for i, ref in enumerate(ref_list):
    ax.plot(
        df_10m.loc[df_10m["reference_short"] == ref, :].MAR_10m_temp,
...
fig.savefig("figures/MAR_comp_1_" + mar_ver + ".png")
...
for data_name in dataset:
    ...
    for i, site in enumerate(site_list):
        ax.plot(
            df_10m.loc[df_10m["site"] == site, :].MAR_10m_temp,
    ...
    ax.set_xlabel("MAR simulated 10 m subsurface temperature ($^o$C)")
    ax.set_ylabel("Observed 10 m subsurface temperature ($^o$C)")
    fig.savefig("figures/MAR_comp_" + data_name + "_" + mar_ver + ".png")
...
for data_name in dataset:
    ...
    for i, site in enumerate(site_list):
        ...
        print(
            ...
            tmp.latitude.mean(),
            tmp.longitude.mean(),
            tmp.temperatureObserved.std(),
            tmp.elevation.mean(),
            tmp.temperatureObserved.mean(),
            tmp.index.year.min(),
            tmp.index.year.max(),
        )
        ax.set_title(site)
    ...
    fig.savefig("figures/MAR_comp_" + data_name + "_" + mar_ver + "_2.png")
...
for year in range(2017, 2020):
    ...
    ds = xr.open_dataset(
        "I:/Baptiste/Data/RCM/MAR/b92/ICE." + str(year) + ".01-12.b92.nc"
    )
    points = [
        (x, y) for x, y in zip(ds["LAT"].values.flatten(), ds["LON"].values.flatten())
    ]
    ...
    ds_ice = ds.TI1[dict(X10_85=MAR_j, Y20_155=MAR_i)].reset_coords()
    ds_ice = ds_ice.load()
    ...
    for i in range(0, 18):
        print(i)
        plt.plot(
            pd.to_datetime(ds_ice.TIME.values),
            ds_ice[dict(OUTLAY=i)].TI1.values,
            label=str(year),
        )
    plt.plot(
        pd.to_datetime(ds_ice.TIME.values),
        ds_ice[dict(OUTLAY=15)].TI1.values,
        label=str(year),
        linewidth=5,
        color="black",
    )
