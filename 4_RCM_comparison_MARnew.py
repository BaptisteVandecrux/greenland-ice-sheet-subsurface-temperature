...
ds = xr.open_dataset('I:/Baptiste/Data/RCM/MAR/'+mar_ver+'/ICE.2007.01-12.'+mar_ver+'.nc',  decode_times = False)
...
for year in  years:
    ...
    if year < 2007:
        continue
    try:
        ds = xr.open_dataset('I:/Baptiste/Data/RCM/MAR/'+mar_ver+'/ICE.'+str(year)+'.01-12.'+mar_ver+'.nc')
    except:
    ...
    for ind in progressbar.progressbar(df_year.index):        
        df_year.loc[ind,'MAR_10m_temp'] = ds.TI1[dict(X10_85=int(df_year.loc[ind].MAR_j),
...
for year in  range(2017,2020):
    ...
    ds = xr.open_dataset('I:/Baptiste/Data/RCM/MAR/b92/ICE.'+str(year)+'.01-12.b92.nc')
    points = [(x, y) for x,y in zip(ds['LAT'].values.flatten(),ds['LON'].values.flatten())]
