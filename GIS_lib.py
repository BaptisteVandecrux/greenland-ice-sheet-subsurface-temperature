...
# Shapefile and template raster should have the same CRS.
    ...
    burned = features.rasterize(shapes=shapes, fill=0, out=out_arr, transform=raster_in.transform)
    return burned==1
...
def plot_geotiff(raster_in, ax, cmap,vmin, vmax,cbar_label):
    tmp2, cbar2 = ep.plot_bands(raster_in.read(1), 
...
def zonal_stat(geotiff_file_name, min_val = None, max_val = None, buffer_shape_file = 'Data/AWS/AWS_buffer.shp'):
    ...
    # Extract zonal stats
    zonal_stat_out = rs.zonal_stats(buffer_shape_file, data_raster,
